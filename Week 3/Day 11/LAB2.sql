...
SELECT  distinct account_id, sum(amount) from bank.loan
GROUP BY account_id
...
#Query 17 / In the loan table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your outp

SELECT `date`, LEFT(`date`,2) AS `year`, count(`date`) FROM bank.loan
WHERE LEFT(`date`,2) = 97
GROUP BY `date`;
